...
def render_countdown(label: str, seconds_left: int) -> None:
    ...
    components.html(
        f"""
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;border:1px solid #d7dee9;border-radius:15px;padding:5px 8px;background:#ffffff;color:#0f172a;box-shadow:0 1px 2px rgba(15,23,42,.08)">
          <div style="font-size:10px;color:#64748b;opacity:1;margin-bottom:0">{safe_label}</div>
...
def render_public_room_creator() -> None:
    ...
    st.markdown('<div class="terminal-note">Buat room sementara dengan link undangan</div>', unsafe_allow_html=True)
...
def render_landing() -> None:
    st.markdown(
        """<div class="terminal-hero">
            <div class="terminal-kicker">Ruang sesi privat yang sederhana</div>
            <h1>AntiTrust</h1>
            <p class="muted">Buat room sementara, salin undangan sekali klik, lalu link otomatis disembunyikan dari halaman.</p>
        </div>""",
        unsafe_allow_html=True,
    )
...
def render_sidebar() -> tuple[bool, int, bool]:
    ...
    auto_refresh = True
    interval = st.sidebar.selectbox("Refresh chat setiap", [2, 5, 8, 10, 15, 30, 60], index=0)
    sound = st.sidebar.toggle("Suara pesan baru", value=True)
    if st.sidebar.button("Refresh sekarang", use_container_width=True):
        st.rerun()
    st.sidebar.caption("Chat diperbarui otomatis. Di HP, fokus diarahkan ke area pesan dan form kirim.")
